...
;
pro integfits,name,nlevel,ps=ps, gif=gif, log=log, conv=conv
loadct,39
;loadct,5
nd= 128 
w2=fltarr(100)
...
if keyword_set(conv)then begin
  psf = psf_Gaussian( NPIXEL= 10, FWHM=5  , /NORMALIZE)
  imconv = smooth(convolve( IdV, psf, FT_PSF = psf_FT ),1)
endif else begin
  imconv = IdV
endelse
;---------------------------
...
;    tv,d
 ...
 filename_fits=name+num(iii)+num(ii)+'.fits'
 ;filename = test.gif
...
;;;;;output to FITS file ;;;
;writefits,filename_fits, finaldisplay 
 writefits,filename_fits, imconv 
